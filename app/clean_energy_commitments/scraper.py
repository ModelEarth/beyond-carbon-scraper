...
def row_parser(row):
    ...
    '''
    titles = ["jurisdiction","clean_energy_commitment",
              "clean_energy_target_percent",
              "carbon_pollution_reduction_goal_percent",
              "electric_vehicle_goals","energy_efficiency_rank"]

    row_dict = lambda row : {titles[index]:row[index] for index in range(len(titles))},
    row_dict['energy_efficiency_rank_num'] = pf.energy_eff_rank_num(row_dict["energy_efficiency_rank"])
    ...
    row_dict.update(pf.carbon_reduction_goal_pct(row_dict["carbon_pollution_reduction_goal_percent"]))
